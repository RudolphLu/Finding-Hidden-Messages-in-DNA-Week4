...
UINT32 random_select(double *prob,double min_prob,UINT32 num)
{
    UINT32 i,sum,j,k;
...
UINT32 GibbsSampler(DNA_INFO_t *dna_info,UINT32 *profile_pattern,UINT8 *best_motifs,UINT32 N) 
{
    ...
    double *m_prob = malloc((size_dna-kmer)*sizeof(double)),c_prob,min_prob=1;
    ...
    for(i=0;i<N;i++){
        ...
        for(j=0,min_prob=1;j<=(size_dna-kmer);j++){              
        ...
        j = random_select(m_prob,min_prob,(size_dna-kmer));
